const translations = {
    hu: {
        navHome: 'Főoldal',
        navMachines: 'Automaták',
        navProducts: 'Termékek',
        navLocations: 'Helyszínek',
        navAbout: 'Rólunk',
        mobileNavHome: 'Főoldal',
        mobileNavMachines: 'Automaták',
        mobileNavProducts: 'Termékek',
        ...
        checkboxLabel: 'Elolvastam és megértettem a fenti nyilatkozatot',
        enterBtn: 'Belépés az oldalra',
        ageVerify: '18 éves kor betöltése szükséges',
        disclaimerScrollHint: '↓ Görgessen le az aljára ↓',
        disclaimerText: `
    <img
        src="images/disclaimer-hu.jpg"
        alt="Jogi Nyilatkozat"
        class="w-full rounded-xl border border-emerald-800/40 shadow-xl"
        draggable="false"
...
`,
        ...
        stat2: 'Prémium Termék',
        stat3: 'Legális és Tanúsított',
        stat4: 'Kártya & NFC Fizetés',
        featuresTitle: 'Miért a Cannabox?',
        featuresSubtitle: 'Intelligens technológia és tanúsított növényi hatóanyagok harmóniája',
        feat1Title: 'Laboratóriumi Tisztaság',
        feat1Desc: 'Minden tétel független harmadik fél által tesztelt. Teljes spektrumú analízis, nulla szennyeződés, bio gazdálkodás.',
        feat1Tag: 'Prémium Garancia',
        feat2Title: 'Azonnali Hozzáférés',
        feat2Desc: 'Nincs sorban állás. Klímaszabályozott automatáink folyamatosan üzemelnek valós idejű készletkövetéssel.',
        feat2Tag: 'Azonnali Hozzáférés',
        ...
        modalTHCLabel: 'THC Tartalom',
        ageRestriction: 'Vásárláshoz 18+ szükséges',

        feat1Check1: '100% Laboratóriumi Minőség',
        feat1Check2: 'EU Megfelelőség',
        feat1Check3: 'Tisztaság Garantált',
        ...
        footerLocationsLink: 'Helyszínek',
        footerAboutLink: 'Rólunk',
        footerPrivacy: 'Adatvédelmi Irányelvek',
        footerTerms: 'Szolgáltatási Feltételek',
        footerCookie: 'Cookie Irányelvek',
        footerCopyright: '© 2026 Cannabox. Minden jog fenntartva.',

        magyarBtn: 'Magyar',
        englishBtn: 'English',
        mapPlaceholder: 'Térkép hamarosan',

        certifiedTitle: 'Tanúsított Termékek',
        certifiedDesc: 'Laboratóriumi tesztelt és ellenőrzött minőség. Minden termék teljesíti a legmagasabb európai szabványokat.',
        alwaysOpenTitle: 'Mindig Nyitva',
        alwaysOpenDesc: '24/7 elérhetőség az összes helyszínen',
        ...
        disclaimerViewerTitle: 'Jogi Nyilatkozat',
        disclaimerViewerClose: 'Bezárás',
        disclaimerZoomHint: 'Kattintson a képre a nagyításhoz',

        // Elakadt Termék
        footerStuckProduct: 'Elakadt Termék Info',
        stuckProductTitle: 'Elakadt Termék Információ',
        stuckProductIntro: 'Ha a terméke elakad az automatában, kérjük, kövesse az alábbi utasításokat:',
        stuckProductClickToZoom: 'Kattintson a nagyításhoz',
    ...
    en: {
        ...
        checkboxLabel: 'I confirm I have read and understood the disclaimer above',
        enterBtn: 'Enter Website',
        ageVerify: 'Must be 18+ years old',
        disclaimerScrollHint: '↓ Scroll to bottom ↓',
        disclaimerText: `
    <img
        src="images/disclaimer-en.png"
...
`,
        ...
        feat2Title: 'Instant Availability',
        feat2Desc: 'No queues, no delays. Our intelligent machines operate continuously with climate control and real-time inventory tracking.',
        feat2Tag: 'Instant Access',
        ...
        modalTHCLabel: 'THC Content',
        ageRestriction: 'Must be 18+ to purchase',

        feat1Check1: '100% Lab Certified',
        ...
        footerLocationsLink: 'Locations',
        footerAboutLink: 'About',
        footerPrivacy: 'Privacy Policy',
        footerTerms: 'Terms of Service',
        footerCookie: 'Cookie Policy',
        footerCopyright: '© 2026 Cannabox. All rights reserved.',
        ...
        disclaimerViewerTitle: 'Legal Disclaimer',
        disclaimerViewerClose: 'Close',
        disclaimerZoomHint: 'Click image to zoom',

        // Stuck Product
        footerStuckProduct: 'Stuck Product Info',
        stuckProductTitle: 'Stuck Product Information',
        stuckProductIntro: 'If your product gets stuck in the vending machine, please follow the instructions below:',
...
let currentLang = sessionStorage.getItem('cannabox_lang') || null;
let disclaimerAccepted = sessionStorage.getItem('cannabox_disclaimer') === 'true';

function selectLanguage(lang) {
    currentLang = lang;
    sessionStorage.setItem('cannabox_lang', lang);

    const langModal = document.getElementById('languageModal');
    if (langModal) {
        langModal.style.opacity = '0';
        langModal.style.pointerEvents = 'none';
        setTimeout(() => {
...
function updateActiveLangButton() {
    ...
    if (huBtn && enBtn && currentLang) {
        if (currentLang === 'hu') {
            huBtn.className = 'px-4 py-2 rounded-lg text-sm font-semibold transition-all bg-emerald-500 text-black shadow-lg shadow-emerald-950/40';
            enBtn.className = 'px-4 py-2 rounded-lg text-sm font-semibold transition-all text-gray-400 hover:text-white';
        } else {
            enBtn.className = 'px-4 py-2 rounded-lg text-sm font-semibold transition-all bg-emerald-500 text-black shadow-lg shadow-emerald-950/40';
            huBtn.className = 'px-4 py-2 rounded-lg text-sm font-semibold transition-all text-gray-400 hover:text-white';
        }
    }
}
...
function updateContent() {
    ...
    elements.forEach(el => {
        const key = el.id;
        if (key === 'disclaimerText' || key === 'disclaimerContent' || key === 'languageModal' || key === 'disclaimerModal') return;
        if (t[key] !== undefined) {
            if (key === 'heroTitle') {
                el.innerHTML = t[key];
            } else {
...
function showDisclaimerModal() {
    ...
    if (modal && !disclaimerAccepted) {
        const activeLang = currentLang || 'en';
        const t = translations[activeLang];
        if (t) {
            const titleEl = document.getElementById('disclaimerTitle');
            const textEl = document.getElementById('disclaimerText');
            const checkLabel = document.getElementById('checkboxLabel');
            const enterBtnEl = document.getElementById('enterBtn');
            const scrollHint = document.getElementById('disclaimerScrollHint');

            if (titleEl) titleEl.textContent = t.disclaimerTitle;
            if (textEl) textEl.innerHTML = t.disclaimerText;
            if (checkLabel) checkLabel.textContent = t.checkboxLabel;
            if (enterBtnEl) enterBtnEl.textContent = t.enterBtn;
            if (scrollHint) scrollHint.textContent = t.disclaimerScrollHint;
        }
        ...
        if (typeof initDisclaimerScroll === 'function') {
...
function ensureLanguageModal() {
    ...
    const html = `
    <div id="languageModal" class="modal-overlay fixed inset-0 z-[100] flex items-center justify-center bg-black/85 backdrop-blur-xl transition-opacity duration-300 hidden opacity-0 pointer-events-none">
        <div class="glass-panel max-w-md w-full mx-4 p-8 rounded-3xl border border-emerald-700/50 shadow-2xl shadow-emerald-950/60 transform transition-all duration-300">
            <div class="text-center mb-8">
                <div class="w-16 h-16 bg-emerald-900/50 border border-emerald-600/40 rounded-2xl flex items-center justify-center mx-auto mb-4 text-emerald-400 shadow-lg">
                    <i data-lucide="globe" class="w-8 h-8"></i>
                </div>
                <h2 id="langTitle" class="text-3xl font-bold text-white mb-2 font-display">Select Language</h2>
                <p id="langSubtitle" class="text-sm text-emerald-200/70">Válasszon nyelvet / Choose your language</p>
            </div>
            <div class="space-y-4">
                <button onclick="selectLanguage('hu')" class="w-full py-4 px-6 rounded-2xl bg-emerald-950/50 hover:bg-emerald-900/60 border border-emerald-800/50 hover:border-emerald-500/60 transition-all flex items-center justify-between group">
                    <div class="flex items-center gap-4">
                        <span class="text-2xl">🇭🇺</span>
                        <span class="text-lg font-bold text-white group-hover:text-emerald-300 font-display">Magyar</span>
                    </div>
                    <i data-lucide="arrow-right" class="w-5 h-5 text-emerald-500 group-hover:translate-x-1 transition-transform"></i>
                </button>
                <button onclick="selectLanguage('en')" class="w-full py-4 px-6 rounded-2xl bg-emerald-950/50 hover:bg-emerald-900/60 border border-emerald-800/50 hover:border-emerald-500/60 transition-all flex items-center justify-between group">
                    <div class="flex items-center gap-4">
                        <span class="text-2xl">🇬🇧</span>
                        <span class="text-lg font-bold text-white group-hover:text-emerald-300 font-display">English</span>
                    </div>
                    <i data-lucide="arrow-right" class="w-5 h-5 text-emerald-500 group-hover:translate-x-1 transition-transform"></i>
                </button>
            </div>
...
function ensureDisclaimerModal() {
    ...
    const html = `
    <div id="disclaimerModal" class="modal-overlay fixed inset-0 z-[99] flex items-center justify-center bg-black/90 backdrop-blur-xl transition-opacity duration-300 opacity-0 pointer-events-none hidden">
        <div class="glass-panel max-w-2xl w-full mx-4 rounded-3xl border border-emerald-700/50 shadow-2xl shadow-emerald-950/80 flex flex-col max-h-[90vh] overflow-hidden">
            <div class="p-6 border-b border-emerald-800/40 flex items-center justify-between bg-[#0a1810]">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-amber-950/60 border border-amber-600/40 flex items-center justify-center">
